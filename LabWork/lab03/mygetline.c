...
char * mygetline( char **buffer,  FILE * infile )
{
  ...
  *buffer = calloc( buffSiz, 1 );
  ...
  do
  {
    ...
    char * result = fgets((*buffer + lineSize), buffSiz, infile);
    ...
    */
    ...
    if (feof(infile)) {
      ...
      lineSize += strlen(result);
    /** DEBUGGING CODE
     *  printf("Size of result = %lu\n", strlen(result));
     *  printf(">>BUFFER = %s\n", *buffer );
     *  printf("buffer_ptr = %p\n", *buffer_ptr);
     *  printf("buffer = %p\n", *buffer);
     *  printf("buffer_ptr += sizeof(result) = %p\n",  buffer_ptr + strlen(result));
     *  printf("Made it passed fgets, with result = %s\n", result);
     */
    ...
    /*
 		...
 		(Ask TA abour strchr)
    */
    char *newline_ptr = strchr(result, '\n');
