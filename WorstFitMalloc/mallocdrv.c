#include <stdio.h>
#include <unistd.h>
#include <stdlib.h>
#include <time.h>

// NOTE:  Change run() at bottom back to main, and uncomment the inclusions of my "mymalloc.h" file just below here.
//        You'll want to run this program from Make, compiling mallocdrv.c, and using this function as the main driver
//        mymalloc.h just has your implementations.

//include your code
#include "mymalloc.h"


//replace malloc here with the appropriate version of mymalloc
#define MALLOC my_worstfit_malloc
//#define MALLOC malloc

//replace free here with the appropriate version of myfree
#define FREE my_free
//#define FREE free

//You can adjust how many things are allocated
#define TIMES 100

//If you want to make times bigger than 100, remove the call to qsort and do something else.
//Then remove this check.
#if TIMES >= 1000
#error "TIMES is too big, qsort will switch to mergesort which requires a temporary malloc()ed array"
#endif

//Do not modify below here
struct tree {
    int data;
    struct tree *left;
    struct tree *right;
};

void freetree(struct tree *root)
{
    if(root->left != NULL)
        freetree(root->left);
    if(root->right != NULL)
        freetree(root->right);
    FREE(root);
}

void randominsert(struct tree *head, struct tree *new)
{
    int way=0;
    struct tree *curr, *prev;
    prev = NULL;
    curr = head;

    while(curr != NULL)
    {
        prev = curr;
        way = rand()%2;
        if(way == 0)
        {
            curr = curr->left;
        }
        else
        {
            curr = curr->right;
        }
    }
    if(way == 0)
        prev->left = new;
    else
        prev->right = new;
}

void printtree(struct tree *head)
{
    struct tree *curr = head;
    if(head == NULL)
        return;

    printtree(curr->left);
    printf("%d\n", curr->data);
    printtree(curr->right);
}

void test1()
{
    int i;

    struct tree *head = (struct tree *)MALLOC(sizeof(struct tree));
    // printf("got past first delcaration");
    head->data = 0;
    head->left = NULL;
    head->right = NULL;

    for(i=1;i<TIMES;i++)
    {
        struct tree *new = (struct tree *)MALLOC(sizeof(struct tree));
        // printf("got past second declaration\n");
        new->data = i;
        new->left = NULL;
        new->right = NULL;
        randominsert(head, new);
    }

    printtree(head);
    // printf("made it past print\n");
    freetree(head);
    // printf("made it past free\n");
}

int comp(const void *a, const void *b)
{
    return *((int *)a) - *((int *)b);
}

void test2()
{
    int *a;
    int i;

    a = (int *)MALLOC(TIMES * sizeof(int));

    for(i=0;i<TIMES;i++)
    {
        a[i] = rand()%TIMES + 1;
    }

    qsort(a, TIMES, sizeof(int), comp);

    for(i=0;i<TIMES;i++)
    {
        printf("%d\n", a[i]);
    }

    FREE(a);

}

int main()
{
    srand((unsigned int)time(NULL));
    printf("original val brk: %p\n", sbrk(0));
    test1();
    printf("brk after  test1: %p\n", sbrk(0));
    test2();
    printf("brk after  test2: %p\n", sbrk(0));
    printf("brk after  test2: %p\n", sbrk(0));


    return 0;
}

