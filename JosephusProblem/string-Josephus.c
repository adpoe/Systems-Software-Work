...
extern CDLL_NODE * searchList( CDLL list, void * target );

void printString( void * data, int mode )
{
    // YOUR CODE HERE
    if(mode == BRIEF) { /* just to shut up the compiler warning about unused arg */
        printf("%s ", (char *) data);
    }
}
...
int compareString( void * data1, void *data2 )
{
    // YOUR CODE HERE
    int compareValue;
    // Go through each string and subtract until one is higher or lower.
    // If one ends before the other it is LESS THAN (<)
    compareValue = strcmp((char*)data1, (char*)data2);
...
int main(int argc, char *argv[])
{
    ...
    while (list.head) /* while list has any members left */
    {
        ...
        printf("\nRESUMING AT: "); printString( currNode->data, BRIEF ); printf( " and skipping %d elements \n", k );
        for ( i=1 ; i<=k ; ++i )
